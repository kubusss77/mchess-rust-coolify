use mchess::{board::Board, piece::PieceColor, pieces::knight::get_controlled_squares_knight};
...
fn test_pinned_knight() {
    let board = Board::from_fen("4k3/4r3/8/8/4N3/8/8/4K3 w - - 0 1");
    let pos = alg("e4");
    let knight = board.get_piece_at(pos.y, pos.x).unwrap();
    let moves = board.get_legal_moves(knight.index);
...
fn test_knight_control() {
    ...
    let pos = alg("d4");
    let knight = board.get_piece_at(pos.y, pos.x).unwrap();
    let control = get_controlled_squares_knight(&knight.to_partial(), &board);
    assert_eq!(control.len(), 8);
}

#[test]
fn test_knight_wrapping() {
    let board = Board::from_fen("6k1/8/8/8/7N/8/8/6K1 w - - 0 1");
    let pos = alg("h4");
    let knight = board.get_piece_at(pos.y, pos.x).unwrap();
    let moves = board.get_legal_moves(knight.index);
