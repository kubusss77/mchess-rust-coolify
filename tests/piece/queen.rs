use mchess::{board::Board, pieces::queen::get_controlled_squares_queen};
...
fn test_diagonal_pinned_queen() {
    let board = Board::from_fen("4k3/6b1/8/8/8/2Q5/8/K7 w - - 0 1");
    let pos = alg("c3");
    let queen = board.get_piece_at(pos.y, pos.x).unwrap();
    let moves = board.get_legal_moves(queen.index);
...
fn test_orthogonal_pinned_queen() {
    let board = Board::from_fen("4k3/4r3/8/8/8/8/4Q3/4K3 w - - 0 1");
    let pos = alg("e2");
    let queen = board.get_piece_at(pos.y, pos.x).unwrap();
    let moves = board.get_legal_moves(queen.index);
...
fn test_queen_check_block() {
    let board = Board::from_fen("4k3/8/8/8/8/8/3Q4/K6r w - - 0 1");
    let pos = alg("d2");
    let queen = board.get_piece_at(pos.y, pos.x).unwrap();
    let moves = board.get_legal_moves(queen.index);
...
fn test_queen_control() {
    ...
    let pos = alg("e4");
    let queen = board.get_piece_at(pos.y, pos.x).unwrap();
    let control = get_controlled_squares_queen(&queen.to_partial(), &board);
